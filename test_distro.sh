...
function set_distro_paths {
	banner
	print_msg "Setting up distro related paths"
...
function distro_setup_user() {
	...
	if [[ "$pd_useradd_answer" == "y" ]]; then
		...
		"${selected_distro_type}"-distro login "$selected_distro" -- /bin/sh -c 'groupadd storage || true'
		"${selected_distro_type}"-distro login "$selected_distro" -- /bin/sh -c 'groupadd wheel || true'
		"${selected_distro_type}"-distro login "$selected_distro" -- /bin/sh -c "useradd -m -g users -s \$(which bash) ${final_user_name}"
		"${selected_distro_type}"-distro login "$selected_distro" -- /bin/sh -c "for g in wheel polkitd audio video storage; do groupadd -f \$g 2>/dev/null || true; usermod -aG \$g ${final_user_name} 2>/dev/null || true; done"
...
function chroot_distro_debian_based_app_installer() {
	cat <<-TOP_EOF >>"$TERMUX_PREFIX/bin/$selected_distro"
		package_install_and_add_to_menu() {
		    ...
		    local install_chrome_mobox=false
		    local new_args=()
		    for arg in "\$@"; do
		        if [[ "\$arg" == "google.chrome" || "\$arg" == "google-chrome" ]]; then
		            install_chrome_mobox=true
		        else
		            new_args+=("\$arg")
		        fi
		    done

		    if [[ "\$install_chrome_mobox" == "true" ]]; then
		        print_msg "Installing Google Chrome via Mobox..."
		        if [[ ! -f "$TERMUX_PREFIX/bin/wine" ]]; then
		            print_failed "Wine/Mobox is not installed. Please set up Wine/Mobox in Termux-Desktop first."
		        else
		            local chrome_installer_url="https://dl.google.com/chrome/install/standalonesources/GoogleChromeStandaloneEnterprise64.msi"
		            local temp_installer="$TERMUX_PREFIX/tmp/ChromeStandaloneSetup64.msi"
		            print_msg "Downloading Google Chrome Windows standalone installer..."
		            mkdir -p "$TERMUX_PREFIX/tmp"
		            if curl -Lf "\$chrome_installer_url" -o "\$temp_installer"; then
		                print_success "Download complete."
		                print_msg "Running installer under Mobox/Wine..."
		                wine "\$temp_installer"
		                print_success "Google Chrome installation initiated under Mobox."
		                
		                # Create a desktop entry for Google Chrome via Mobox/Wine
		                local desktop_file="$TERMUX_PREFIX/share/applications/google-chrome-mobox.desktop"
		                cat <<-CHROME_EOF > "\$desktop_file"
							[Desktop Entry]
		                ...
		                update-desktop-database "$TERMUX_PREFIX/share/applications" 2>/dev/null || true
		                print_success "Created desktop shortcut: Google Chrome (Mobox)"
		            else
		                print_failed "Failed to download Google Chrome installer."
		            fi
		        fi
		    fi
...
function add_chroot_distro_function_caller() {
	cat <<-EOF >>"$TERMUX_PREFIX/bin/$selected_distro"
		...
		function help_distro_container() {
			...
			Options:
			install <packagename>    - install packages and add them to termux desktop (e.g. google.chrome via Mobox)
